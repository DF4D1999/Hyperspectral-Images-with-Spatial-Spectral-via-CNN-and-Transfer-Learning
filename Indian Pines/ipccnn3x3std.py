...
for p in range(np.shape(images)[2]):
    for j in range(np.shape(images)[0]):
        for k in range(np.shape(images)[1]):
           if((j==0) & (k==0)):
                new_images[j][k][p] = (images[j][k][p]     + images[j][k+1][p]    +
                                       images[j+1][k][p]   + images[j+1][k+1][p]) / 4
           elif((j==0 )&( k==144)):
               new_images[j][k][p] = (images[j][k][p]      + images[j+1][k-1][p]    +
                                      images[j][k-1][p]    + images[j+1][k][p])     / 4
           elif((k==0)& (j==144)):
               new_images[j][k][p] = (images[j-1][k][p]    + images[j-1][k+1][p]  +
                                      images[j][k][p]      + images[j][k+1][p])   / 4
           elif((k==144) &( j==144)):
               new_images[j][k][p] = (images[j-1][k-1][p]  + images[j-1][k][p]    +
                                      images[j][k-1][p]    + images[j][k][p])     / 4
           elif((j==0) & (0<k<144)):
               new_images[j][k][p] = (images[j][k-1][p]    + images[j][k][p]   + images[j][k+1][p]    +
                                      images[j+1][k-1][p]  + images[j+1][k][p] + images[j+1][k+1][p]) / 6
           elif((j==144) & (0<k<144)):
               new_images[j][k][p] = (images[j-1][k-1][p]  + images[j-1][k][p] + images[j-1][k+1][p]  +
                                      images[j][k-1][p]    + images[j][k][p]   + images[j][k+1][p])   / 6
           elif((k==0) & (0<j<144)):
               new_images[j][k][p] = (images[j-1][k][p]    + images[j-1][k+1][p]   +
                                      images[j][k][p]      + images[j][k+1][p]     +
                                      images[j+1][k][p]    + images[j+1][k+1][p])  / 6
           elif((k==144) &( 0<j<144)):
               new_images[j][k][p] = (images[j-1][k-1][p]  + images[j-1][k][p]     + 
                                      images[j][k-1][p]    + images[j][k][p]       + 
                                      images[j+1][k-1][p]  + images[j+1][k][p])    / 6
           else:#middle 
                new_images[j][k][p] = (images[j-1][k-1][p] + images[j-1][k][p] + images[j-1][k+1][p]  +
                                       images[j][k-1][p]   + images[j][k][p]   + images[j][k+1][p]    +
                                       images[j+1][k-1][p] + images[j+1][k][p] + images[j+1][k+1][p]) / 9
...
for p in range((np.shape(images)[2]*2)):#calculate to page 447.
    if(p % 2 != 0):
        ...
        for j in range(np.shape(images)[0]):
            for k in range(np.shape(images)[1]):
               if((0<j<144)&(0<k<144)):
                    stdarray = (images[j-1][k-1][p//2], images[j-1][k][p//2], images[j-1][k+1][p//2], # '//' for no decimal preserve.
                                images[j][k-1][p//2],   images[j][k][p//2],   images[j][k+1][p//2],
                                images[j+1][k-1][p//2], images[j+1][k][p//2], images[j+1][k+1][p//2]) #p = 1, p(std) comes from p = 0; p = 3, p(std) comes from p = 1;
                    new_images[j][k][p] = np.std(stdarray)
               elif((j==0) & (k==0)):
                    stdarray = (images[j][k][p//2],     images[j][k+1][p//2],
                                images[j+1][k][p//2],   images[j+1][k+1][p//2]) 
                    new_images[j][k][p] = np.std(stdarray)
               elif((j==0 )&( k==144)):
                   stdarray = (images[j][k-1][p//2],    images[j][k][p//2],
                               images[j+1][k-1][p//2],  images[j+1][k][p//2])
                   new_images[j][k][p] = np.std(stdarray)
               elif((k==144) &( j==144)):
                   stdarray = (images[j-1][k-1][p//2],  images[j-1][k][p//2], 
                               images[j][k-1][p//2],    images[j][k][p//2]) 
                   new_images[j][k][p] = np.std(stdarray)
               elif((k==0)& (j==144)):
                   stdarray = (images[j-1][k][p//2],    images[j-1][k+1][p//2], 
                               images[j][k][p//2],      images[j][k+1][p//2])
                   new_images[j][k][p] = np.std(stdarray)
               elif((j==0) & (0<k<144)):
                   stdarray = (images[j][k-1][p//2],    images[j][k][p//2],   images[j][k+1][p//2],
                               images[j+1][k-1][p//2],  images[j+1][k][p//2], images[j+1][k+1][p//2])
                   new_images[j][k][p] = np.std(stdarray)
               elif((j==144) & (0<k<144)):
                   stdarray = (images[j-1][k-1][p//2],  images[j-1][k][p//2], images[j-1][k+1][p//2],
                               images[j][k-1][p//2],    images[j][k][p//2],   images[j][k+1][p//2])
                   new_images[j][k][p] = np.std(stdarray)
               elif((k==0) & (0<j<144)):
                   stdarray = (images[j-1][k][p//2],    images[j-1][k+1][p//2], 
...
sio.savemat('Indian_pines_corrected3x3std.mat',{'indian_pines_corrected':images})'''
images = sio.loadmat('Indian_pines_corrected3x3std.mat')['indian_pines_corrected']
labels = sio.loadmat('Indian_pines_gt')['indian_pines_gt']
...
one = np.where(no0labels == 0)
no0labels = np.delete(no0labels,one)
no0images = np.delete(no0images,one,axis=0)
for i in range(no0labels.size):
    no0labels[i] -= 1
    
one = np.where(no0labels == 2)
no0labels = np.delete(no0labels,one)
no0images = np.delete(no0images,one,axis=0)
for i in range(no0labels.size):
    if(no0labels[i]>2):
